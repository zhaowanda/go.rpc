...
func (client *Client) send(call *Call) {
	...
	err := client.codec.WriteRequest(cxt, &client.request, call.Args)
	if err != nil {
		client.mutex.Lock()
...
func (client *Client) input() {
	var err error
	var response gorpc.Response
	cxt := NewMapContext(context.Background())
	for err == nil {
		response = gorpc.Response{}
		err = client.codec.ReadResponseHeader(cxt, &response)
		if err != nil {
			break
		}
		seq := response.Seq
		...
		case call == nil:
			...
			err = client.codec.ReadResponseBody(cxt, nil)
			if err != nil {
		...
		case response.Error != "":
			...
			call.done()
		default:
			err = client.codec.ReadResponseBody(cxt, call.Reply)
			if err != nil {
...
func NewClient(conn io.ReadWriteCloser) *Client {
	client := gob.NewClientCodec(conn)
